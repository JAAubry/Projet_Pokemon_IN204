...
bool Joueur::peutContinuer() const {
    // Un joueur peut continuer s'il a un Pokemon actif OU s'il peut en placer un
    return aPokemonActif() || peutPlacerPokemonActif();
}

bool Joueur::aPerdu() const {
    // Ne peut pas piocher au début de son tour (deck vide)
    // N'a plus de Pokémon actif ET ne peut pas en placer un
...
void Joueur::setDeck(std::vector<std::unique_ptr<CarteBase>> deck) {
    ...
    piocher(std::min(static_cast<int>(NOMBRE_CARTES_DEBUT), static_cast<int>(TAILLE_MAX_MAIN))); 
}
...
void Joueur::piocher(int n) {
    ...
    for (int i = 0; i < n && !pioche.empty(); ++i) {
        ...
        if (main.size() >= TAILLE_MAX_MAIN) {
            std::cout << nom << " a la main pleine (" 
                      << TAILLE_MAX_MAIN << " cartes maximum)!\n";
            std::cout << "   Impossible de piocher plus de cartes.\n";
            break;  // Arrêter de piocher
        }
        
        main.push_back(std::move(pioche.back()));
        pioche.pop_back();
        cartesPiochees++;
        std::cout << nom << " pioche une carte. (Main: " 
                  << main.size() << "/" << TAILLE_MAX_MAIN << ")\n";
    }
    
    if (cartesPiochees == 0 && !pioche.empty()) {
        std::cout << nom << " ne peut pas piocher : main pleine!\n";
    }
...
void Joueur::jouerCarte(std::size_t index) {
    ...
    if (main[index]->getType() == TypeCarte::Pokemon) {
        ...
        } else if (banc.size() < TAILLE_BANC) {  // Limite standard à 3 Pokémon sur le banc
            std::cout << pokemon->getNom() << " est placé sur le banc.\n";
            banc.push_back(std::move(pokemon));
        } else {
            std::cout << "Le banc est plein ("<< TAILLE_BANC<<" Pokémon maximum)!\n";
            return; 
        }
    } else {
        // Carte Dresseur
...
void Joueur::attaquer(Joueur& adversaire) {
    ...
    // Si le Pokemon adverse est K.O., il doit être retiré
    if (adversaire.actif->estKO()) {
        std::cout << adversaire.actif->getNom() << " de " 
                  << adversaire.nom << " est mis K.O.!\n";
        adversaire.actif.reset();
        
        // L'adversaire doit placer un nouveau Pokemon actif s'il en a
        if (!adversaire.banc.empty()) {
            std::cout << adversaire.nom << " doit choisir un nouveau Pokemon actif du banc.\n";
        }
    }
}
...
void Joueur::batterEnRetraite(std::size_t indexBanc) {
    ...
    if (!actif->peutRetraite()) {
        std::cout << actif->getNom() 
                  << " n'a pas assez d'energie pour battre en retraite! (Coût: " 
...
void Joueur::finTour() {
    // Au cas ou pour d'éventuels effets de fin de tour
}
