...
void afficherEtatComplet(const Joueur& joueur, const Joueur& adversaire) {
    ...
    if (adversaire.aPokemonActif()) {
        std::cout << "Pokemon actif: ";
        adversaire.getPokemonActif()->afficher();
    } else {
        std::cout << "Aucun Pokemon actif\n";
    }
    std::cout << "Banc: " << adversaire.getBanc().size() << " Pokemon\n";
}
...
void afficherMenu() {
    ...
    std::cout << "│ 5. Voir l'etat du jeu            │\n";
    std::cout << "│ 6. Passer le tour                │\n";
    std::cout << "|                                  |\n";
    std::cout << "Choix: ";
}

// ----------- REMPLACEMENT POKEMON ACTIF -----------
void remplacerPokemonActif(Joueur& joueur) {
    if (joueur.getBanc().empty()) {
        std::cout << joueur.getNom() << " n'a plus de Pokemon sur le banc!\n";
        std::cout << joueur.getNom() << " ne peut plus continuer!\n";
        return;
    }
    
    std::cout << "\n" << joueur.getNom() << " doit choisir un nouveau Pokemon actif:\n";
    ...
    std::cout << "Choisissez un Pokemon (0-" << (banc.size() - 1) << "): ";
...
int main() {
    ...
    std::cout << "   JEU DE CARTES POKEMON  \n";
    ...
    while (joueur1.peutContinuer() && joueur2.peutContinuer()) {
        ...
        for (auto* joueur : {&joueur1, &joueur2}) {
            ...
            if (!joueur->aPokemonActif()) {
                std::cout << joueur->getNom() << " n'a plus de Pokemon!\n";
                break;
            }
            ...
            while (!tourTermine) {
                ...
                switch (choix) {
                    ...
                    case 4: {  // Battre en retraite
                        if (joueur->getBanc().empty()) {
                            std::cout << " Vous n'avez pas de Pokemon sur le banc!\n";
                            break;
                        }
                        
                        std::cout << "Choisissez un Pokemon du banc (0-" 
